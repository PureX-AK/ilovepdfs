...
export default function PDFToPDFA() {
  ...
  const handleConvert = async () => {
    ...
    const toastId = showLoading('Converting to PDF/A...');
    ...
    try {
      ...
      setTimeout(() => {
        ...
        setProgress(0);
        updateToSuccess(toastId, 'PDF converted to PDF/A successfully! Download started.');
      }, 500);
    } catch (error) {
      clearInterval(progressInterval);
      setIsProcessing(false);
      setProgress(0);
      updateToError(toastId, 'An error occurred while converting to PDF/A. Please try again.');
  ...
  const convertToPDFA = async () => {
    ...
    try {
      ...
      // Create a new PDF document optimized for PDF/A compliance
      const pdfA = await PDFDocument.create();
      
      // Copy all pages
      const pageIndices = pdf.getPageIndices();
      const copiedPages = await pdfA.copyPages(pdf, pageIndices);
      copiedPages.forEach((page) => pdfA.addPage(page));
      ...
      link.href = url;
      link.download = selectedFile.name.replace('.pdf', '') + '_pdfa.pdf';
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-8">
            <p className="text-sm text-blue-800">
              <strong>About PDF/A:</strong> PDF/A is an ISO-standardized version of PDF designed for long-term archiving. It ensures that documents will be readable in the future by embedding all necessary fonts and metadata. This is a basic conversion - for full PDF/A compliance validation, use specialized tools.
            </p>
          </div>
