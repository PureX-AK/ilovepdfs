...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Get the Python script path - use spire-pdf version for EditPDF
    const scriptPath = path.join(process.cwd(), 'scripts', 'pdf_to_html_spire.py');
    
    // Check if script exists
    if (!fs.existsSync(scriptPath)) {
      throw new Error('PDF to HTML conversion script not found. Please ensure scripts/pdf_to_html_spire.py exists.');
    }
    ...
    try {
      ...
      // Check if it's a SkiaSharp/native dependency error
      if (execError.stderr && (execError.stderr.includes('SkiaSharp') || execError.stderr.includes('libSkiaSharp') || execError.stderr.includes('DllNotFound'))) {
        throw new Error(
          'spire-pdf requires native SkiaSharp dependencies. Please install Visual C++ Redistributable or use PyMuPDF instead.'
        );
      }
      // Check if it's a Python/library error
      if (execError.stderr && execError.stderr.includes('spire-pdf')) {
        throw new Error(
          'spire-pdf library not installed. Please install it with: pip install spire-pdf'
        );
      }
    ...
    return NextResponse.json(
      { 
        success: false, 
        error: error.message || 'Failed to convert PDF to HTML. Please ensure Python 3.8+ and spire-pdf library are installed.' 
      },
      { status: 500 }
    );
  } finally {
