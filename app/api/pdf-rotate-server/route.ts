...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const applyToAll = (formData.get('applyToAll') as string) === 'true';
    ...
    if (![90, 180, 270].includes(rotation)) {
      return NextResponse.json({ success: false, error: 'Invalid rotation angle. Must be 90, 180, or 270.' }, { status: 400 });
    }
    ...
    const scriptPath = path.join(process.cwd(), 'scripts', 'pdf_rotate.py');
    if (!fs.existsSync(scriptPath)) {
      return NextResponse.json({ success: false, error: 'Rotate script not found' }, { status: 500 });
    }

    try {
      const { stdout, stderr } = await execFileAsync(
        pythonCmd,
        [scriptPath, tempInputPath, tempOutputPath, rotation.toString(), applyToAll.toString()],
        { timeout: 60000, maxBuffer: 10 * 1024 * 1024 }
      );

      if (stderr && stderr.includes('ERROR:')) {
        return NextResponse.json({ success: false, error: stderr.split('ERROR:')[1]?.trim() || 'Rotation failed' }, { status: 500 });
      }

      if (!fs.existsSync(tempOutputPath)) {
        return NextResponse.json({ success: false, error: 'Rotation failed: Output file was not created' }, { status: 500 });
      }

      const stats = await fs.promises.stat(tempOutputPath);
      if (stats.size === 0) {
        return NextResponse.json({ success: false, error: 'Rotation failed: Output file is empty' }, { status: 500 });
      }
      ...
      if (fileHeader !== '%PDF') {
        ...
        return NextResponse.json({ success: false, error: `Rotation failed. Output is not a valid PDF: ${errorContent.substring(0, 200)}` }, { status: 500 });
      }
      ...
      console.error('Rotation processing error:', error);
      ...
      if (error.message && error.message.includes('PyMuPDF')) {
        ...
        errorMessage = `Rotation failed: ${error.message}`;
      }
