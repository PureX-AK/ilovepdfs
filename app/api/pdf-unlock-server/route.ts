...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const scriptPath = path.join(process.cwd(), 'scripts', 'pdf_unlock.py');
    if (!fs.existsSync(scriptPath)) {
      return NextResponse.json({ success: false, error: 'Unlock script not found' }, { status: 500 });
    }

    // Build command arguments
    const args = [scriptPath, tempInputPath, tempOutputPath];
    if (password.trim()) {
      args.push(password);
    }
    ...
    try {
      ...
      if (stderr && stderr.includes('ERROR:')) {
        return NextResponse.json({ success: false, error: stderr.split('ERROR:')[1]?.trim() || 'Unlocking failed' }, { status: 500 });
      }

      if (!fs.existsSync(tempOutputPath)) {
        return NextResponse.json({ success: false, error: 'Unlocking failed: Output file was not created' }, { status: 500 });
      }

      const stats = await fs.promises.stat(tempOutputPath);
      if (stats.size === 0) {
        return NextResponse.json({ success: false, error: 'Unlocking failed: Output file is empty' }, { status: 500 });
      }
      ...
      if (fileHeader !== '%PDF') {
        ...
        return NextResponse.json({ success: false, error: `Unlocking failed. Output is not a valid PDF: ${errorContent.substring(0, 200)}` }, { status: 500 });
      }
      ...
      let errorMessage = 'Unlocking failed';
      if (error.message && error.message.includes('PyMuPDF')) {
        ...
        errorMessage = error.stderr.split('ERROR:')[1]?.trim() || 'Unlocking failed';
      } else if (error.message) {
        errorMessage = `Unlocking failed: ${error.message}`;
      }
